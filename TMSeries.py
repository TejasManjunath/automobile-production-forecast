...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 23 22:58:38 2026
...
def get_backtest_origins(series, min_train_months=24):
    n = len(series)
...
def plot_forecast(series, origins, actuals_all, forecasts_all, model_name="Model"):
    ...
    # ── Zaman serisi ─────────────────────────────────────────────
    ax1 = axes[0]
    ax1.plot(dates, actuals_all,   label='Actual',   color='steelblue',  linewidth=2)
    ax1.plot(dates, forecasts_all, label='Forecast', color='darkorange', linewidth=2, linestyle='--')
    ax1.set_title("Actual vs Forecast (Zaman Serisi)")
    ...
    # ── Scatter ──────────────────────────────────────────────────
    ax2 = axes[1]
    ax2.scatter(actuals_all, forecasts_all, color='steelblue', alpha=0.7, edgecolors='white')
...
# ── SARIMA — sabit parametreler (ACF/PACF analizinden) ───────────────────────

def run_arima(series, origins, model_name="SARIMA"):
    """
    Parametreler ACF/PACF ve diagnostik testlerden manuel belirlendi:
      - LB testi p=0.0998 (>0.05) ile (1,0,1)(0,1,2,12) geçti
      - auto_arima bu veri setinde tutarsız sonuç verdiği için bypass edildi
    """
    print(f"\n{'='*60}")
    print(f"Running {model_name}...")
...
# ── Forecast Combination (HW + SARIMA) ───────────────────────────────────────
...
def run_combination(series, origins,
    ...
    """Equal-weight (0.5/0.5) combination — weights NOT chosen by leaderboard."""
    print(f"\n{'='*60}")
    print(f"Running {model_name}...")
...
def vda_comparison(series, model_name, order, seasonal_order=None):
    print(f"\n{'='*60}")
    ...
    for year in [2024, 2025]:
        ...
        try:
            model = SARIMAX(train, order=order,
                            ...
                            enforce_invertibility=False)
            fit        = model.fit(disp=False)
            our_annual = float(fit.forecast(12).sum())
...
if __name__ == "__main__":
    ...
    origins = get_backtest_origins(series, min_train_months=24)
    ...
    # ── SARIMA ───────────────────────────────────────────────────────────────
    sa_df, sa_act, sa_fcast, sa_res, sa_order, sa_seas = run_arima(series, origins)
    ...
    plot_forecast(series, origins, comb_act, comb_fcast,
                  model_name="Forecast Combination (HW + SARIMA)")

    # ── VDA Karşılaştırması ──────────────────────────────────────────────────
    vda_comparison(series, "SARIMA", order=sa_order, seasonal_order=sa_seas)
